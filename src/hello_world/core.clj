...
(defn sqlquery [name empfaenger]
  ...
  (jdbc/execute! mysql-db ["update transaktionen_geberkonten2nehmerkonten set betrag = betrag * (update geberkonten set stand = stand - 25 where geberkonten.nr = ? AND stand > 0) where sender=?" name name])
