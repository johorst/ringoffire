...
(defn sqlquery [name empfaenger]
  ...
  (jdbc/execute! mysql-db ["update transaktionen_geberkonten2nehmerkonten set valid = (select valid from geberkonten where geberkonten.nr =?) where sender=?" name name])
